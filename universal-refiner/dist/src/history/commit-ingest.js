...
export class CommitIngester {
     ...
     */
    static async ingestLatest(repoPath, limit = 10) {
     ...
     */
    async ingest(repoPath, limit = 10) {
        try {
     ...
     */
    getLatestCommits(repoPath, limit, lastSha = null) {
        ...
        for (const line of lines) {
            ...
            // Get changed files
            const filesOutput = execSync(`git show --name-only --pretty=format: ${sha}`, { cwd: repoPath, encoding: "utf-8" });
            ...
            try {
                const statOutput = execSync(`git show --shortstat --pretty=format: ${sha}`, { cwd: repoPath, encoding: "utf-8" }).trim();
                if (statOutput) {
