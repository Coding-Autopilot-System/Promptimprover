...
export class PromptRefinerServer {
    server;
    rootPath;
    samplingUnavailableReason = null;
    eventStore;
    backgroundAutonomy = null;
    constructor(rootPath = ".") {
        this.rootPath = rootPath;
        this.eventStore = EventStore.getInstance();
        this.server = new Server({ name: "prompt-refiner", version: getPackageVersion() }, { capabilities: { tools: {}, logging: {}, experimental: { sampling: {} } } });
    ...
    async requestModelText(taskName, userPrompt, maxTokens) {
        if (this.samplingUnavailableReason) {
            return null;
        }
        try {
    ...
    async lintSemantic(prompt, ctx) {
        CommandCenterDashboard.log(`Executing Semantic Intelligence Analysis...`);
        const responseText = await this.requestModelText("Semantic analysis", `Act as a senior software architect. Analyze this user prompt in the context of the current project and identify "semantic gaps" where the prompt is vague, misaligned with the tech stack, or missing critical engineering details.
...
Output ONLY the JSON array. If no gaps, return [].`, 1000);
        if (!responseText) {
    ...
    getAgentName(request) {
        return request.params?._meta?.progressToken ||
            request.params?._meta?.agentName ||
            "User CLI";
    }
    setupToolHandlers() {
            ...
            try {
                ...
                switch (request.params.name) {
                    ...
                    case "discover_rules": {
                        ...
                        const responseText = await this.requestModelText("Rule discovery", "Analyze this project context: " + JSON.stringify(ctx) + ". Propose 3 high-quality engineering mandates (rules) that would improve this specific codebase. Format each as a JSON object with 'id', 'category', and 'description'. Output ONLY the JSON array.", 1000);
                        if (!responseText) {
                    ...
                    case "proactive_suggest": {
                        ...
                        const responseText = await this.requestModelText("Proactive suggestion", `Context: ${JSON.stringify(ctx)}. Task: "${prompt}". 3-step plan?`, 500);
                        return {
                            content: [{
                                    type: "text",
                                    text: responseText
                    ...
                    case "generate_agent_onboarding": {
                        ...
                        const responseText = await this.requestModelText("Agent onboarding generation", `Context: ${JSON.stringify(ctx)}. Create AGENTS.md.`, 1000);
                        return {
                            content: [{
                                    type: "text",
    ...
    async run() {
        ...
        this.backgroundAutonomy = new BackgroundAutonomyService(this.rootPath, this.requestModelText.bind(this));
