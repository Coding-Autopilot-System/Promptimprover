...
export class NeuralSnippets {
    static symbolIndex = new Index({ tokenize: "forward", resolution: 9 });
    static contentIndex = new Index({ tokenize: "forward", resolution: 5 });
    ...
    static extractSymbolBlock(lines, startIndex, type) {
        const startLine = lines[startIndex];
        const result = [startLine];
        if (type === "py") {
            ...
            for (let i = startIndex + 1; i < lines.length; i++) {
                ...
                if (openBrackets <= 0 && (line.includes("}") || result.length > 5))
                    break;
                if (result.length > 50)
                    break;
            }
    ...
    static parseSymbols(content, filePath) {
        const symbols = [];
    ...
    static async initialize(rootPath = ".") {
        ...
        for (const filePath of files) {
            ...
            for (const sym of symbols) {
                const doc = { id: id++, filePath, content: sym.content, symbolName: sym.symbolName, symbolType: sym.symbolType };
            ...
            for (let i = 0; i < lines.length; i += 15) {
                ...
                if (chunk.trim().length > 100) {
                    const doc = { id: id++, filePath, content: chunk, symbolType: "chunk" };
                    this.contentIndex.add(doc.id, doc.content);
                    this.store.set(doc.id, doc);
                }
            }
    ...
    static async search(query, rootPath = ".", limit = 5) {
