export class PromptRefiner {
    static calculateGain(original, refined, ctx) {
    ...
    static refine(originalPrompt, ctx, answers, promptId) {
        ...
        if (ctx.orm)
            refined += `- **ORM/Database**: ${ctx.orm}\n`;
        if (ctx.styling)
            refined += `- **Styling**: ${ctx.styling}\n`;
        if (ctx.cloud)
            refined += `- **Cloud Environment**: ${ctx.cloud}\n`;
