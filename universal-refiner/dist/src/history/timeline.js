...
export class TimelineProvider {
    ...
    `).all(limit);
        const unified = [
            ...prompts.map((p) => ({ ...p, details: { intent: p.details } })),
            ...commits.map((c) => ({ ...c, details: { files: JSON.parse(c.details || "[]") } })),
            ...events.map((e) => ({ ...e, details: JSON.parse(e.details || "{}") }))
        ];
