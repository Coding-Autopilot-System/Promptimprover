...
export interface ProjectContext {
  ...
  customMandates?: string[];
  predictiveLessons?: any[];
}

export class ArchitecturalScout {
  static async detectPatterns(rootPath: string = "."): Promise<string[]> {
    const patterns: string[] = [];
    if (!fs.existsSync(rootPath)) return patterns;
...
export class NodeDetector {
  static async detect(rootPath: string = "."): Promise<Partial<ProjectContext>> {
    ...
    try {
      ...
      if (deps["express"] || deps["hono"]) context.framework = deps["express"] ? "Express" : "Hono";
      if (deps["@nestjs/core"]) context.framework = "NestJS";
      if (deps["react"]) context.framework = "React";
      if (deps["next"]) context.framework = "Next.js";

      if (deps["prisma"]) context.orm = "Prisma";
      if (deps["drizzle-orm"]) context.orm = "Drizzle";
      if (deps["typeorm"]) context.orm = "TypeORM";
      if (deps["sequelize"]) context.orm = "Sequelize";

      if (deps["tailwindcss"]) context.styling = "Tailwind CSS";
      if (deps["styled-components"]) context.styling = "Styled Components";

      if (deps["@azure/functions"]) context.cloud = "Azure Functions";
      if (deps["@aws-sdk/client-lambda"]) context.cloud = "AWS Lambda";
...
export class PythonDetector {
  static async detect(rootPath: string = "."): Promise<Partial<ProjectContext>> {
    ...
    let content = "";
    try {
      if (hasReqs) content = fs.readFileSync(path.join(rootPath, "requirements.txt"), "utf-8");
      if (hasPyProject) content += fs.readFileSync(path.join(rootPath, "pyproject.toml"), "utf-8");
    } catch (error) {
    ...
    if (c.includes("fastapi")) context.framework = "FastAPI";
    if (c.includes("django")) context.framework = "Django";
    if (c.includes("flask")) context.framework = "Flask";

    if (c.includes("sqlalchemy")) context.orm = "SQLAlchemy";
    if (c.includes("tortoise-orm")) context.orm = "Tortoise ORM";
    if (c.includes("peewee")) context.orm = "Peewee";

    if (c.includes("pytest")) context.testing = "Pytest";
    if (c.includes("unittest")) context.testing = "Unittest";
