...
export class NeuralSnippets {
  private static index = new Index({
    tokenize: "forward",
    resolution: 9
  });
  ...
  static async search(query: string, limit = 5): Promise<Snippet[]> {
    ...
    if (mainResults) {
      for (const id of mainResults as number[]) {
    ...
    if (snippets.length < limit) {
      for (const word of keywords) {
        ...
        if (wordResults) {
          for (const id of wordResults as number[]) {
            if (!seenIds.has(id)) {
              const doc = this.store.get(id);
              if (doc) {
                snippets.push(doc);
